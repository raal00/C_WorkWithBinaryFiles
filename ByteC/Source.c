...
int main(void)
{
	...
	while (1)
	{
		...
		{
			printf("[ERROR] Ошибка в исходном файле. Длина строки должна быть > 0\n");
		}
		...
		if (result != 1)
		{
			printf("[ERROR] Не удалось считать количество букв строки из файла\n");
			break;
		}
		result = fread(&inputFormat.sumOfLetterCodes, sizeof(int), 1, OutputBinFileThread);
		if (result != 1)
		{
			printf("[ERROR] Не удалось считать сумму кодов символов строки из файла\n");
			break;
		}
		...
		{
			printf("[WARNING] Не удалось считать всю строку из файла. Прочитано %d байт\n", result);
